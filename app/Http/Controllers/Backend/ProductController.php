...
class ProductController extends Controller
{
    ...
    public function StoreProduct(Request $request){
        ...
        Image::make($image)->resize(900,1300)->save('upload/products/thumbnail/'.$name_gen);
        $save_url = 'upload/products/thumbnail/'.$name_gen;

        $image2 = $request->file('size_chart');
        if(!empty($image2)){
            $name_gen2 = hexdec(uniqid()).'.'.$image2->getClientOriginalExtension();
            Image::make($image2)->resize(900,1300)->save('upload/products/sizeChart/'.$name_gen2);
...
>>>>>>> parent of 1f37d31... Merge branch 'master' into Development
        ...
        foreach ($images as $img) {
            ...
            Image::make($img)->resize(900,1300)->save('upload/products/multi-image/'.$make_name);
